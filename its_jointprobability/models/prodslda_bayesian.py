...
import pyro.optim
from pyro.poutine.handlers import ScaleMessenger
import torch
...
class Encoder_Bayes(PyroModule):
    ...
    def forward(self, docs: torch.Tensor, scale: ScaleMessenger) -> torch.Tensor:
        ic(docs.shape)
        with scale:
            logtheta_loc, logtheta_logvar = self.encoder(docs).split(
                self.num_topics, -1
            )
            ic(logtheta_loc.shape)
            logtheta_scale = (0.5 * logtheta_logvar).exp()

            logtheta = pyro.sample(
                "logtheta", dist.Normal(logtheta_loc, logtheta_scale).to_event(2)
            )
            ic(logtheta.shape)
            return logtheta


class Decoder_Bayes(PyroModule):
    def __init__(self, voc_size: int, num_topics: int, dropout: float):
        super().__init__()
...
class ProdSLDA_Bayes(Model):
    ...
    def __init__(
        ...
        self.decoder = Decoder_Bayes(voc_size, num_topics, dropout)
        self.encoder = Encoder_Bayes(voc_size, num_topics, layers, dropout)

        self.decoder_guide = pyro.infer.autoguide.AutoDiagonalNormal(self.decoder)
        self.encoder_guide = pyro.infer.autoguide.AutoDiagonalNormal(self.encoder)
    ...
    def model(
        ...
        docs_plate = pyro.plate("documents", batch_size, dim=-1)
        ...
        ic(batch_size)
        logtheta = self.encoder(docs, scale)
    ...
    def guide(
        ...
        logtheta_q = self.encoder_guide(docs, scale)["logtheta"]
        theta_q = F.softmax(logtheta_q, -1)

        self.decoder_guide(theta_q)
...
def import_data(
    path: Path,
) -> tuple[ProdSLDA_Bayes, dict[int, str], list[str], torch.Tensor, torch.Tensor]:
    args = torch.load(path / "prodslda_args")
    kwargs = torch.load(path / "prodslda_kwargs")
    prodslda = ProdSLDA_Bayes(*args, **kwargs).to(device)
...
def retrain_model(path: Path, n: Optional[int] = None) -> ProdSLDA_Bayes:
    ic.disable()
    ...
    prodslda.run_svi(
        ...
        elbo=pyro.infer.TraceGraph_ELBO(num_particles=1),
        initial_lr=0.001,
        batch_size=2000,
    )

    torch.save(prodslda, path / "prodslda_new")
    pyro.get_param_store().save(path / "pyro_store_new")

    return prodslda


def train_here_qualities() -> ProdSLDA_Bayes:
    path = Path.cwd() / "data"
    n = 6000
